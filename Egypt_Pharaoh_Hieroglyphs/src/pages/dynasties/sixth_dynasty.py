...
layout = html.Div(
    children = [
        ...
        html.H6('2345 - 2181 BC, belongs to the "Old Kingdom"'),
        html.Br(),
        html.Div(id="grid-output_6"),
        dbc.Modal(id="custom-component-img-modal_6", size="s"),
        html.Div(grid_note),
        html.Br(),
    ],
    style={
...
@callback(
    Output("grid-output_6", "children"),
    Input("store", "data"),
)
def update(store):
    if store == {}:
        return "Have you selected sixth dynasty dropdown item? Dataset is empty ..."
        
    df_dyn = pd.DataFrame(store)
    logger.debug('-----  in sixth dyn: callback update(store): store: %s -----', store)

    return dag.AgGrid(
                id='sixth_dynasty_img_dag',
...
@callback(
    Output("custom-component-img-modal_6", "is_open"),
    Output("custom-component-img-modal_6", "children"),
    Input('sixth_dynasty_img_dag', "cellRendererData"),
)
def show_change(data):
    ''' Shows image or cartouche on additional screen after click on such element '''
    if data:
        logger.debug('--- in sixth dyn: callback show_change(data):\n  ==> data: %s  -----', data)
