...
layout = html.Div(
    children = [
        ...
        html.H6('2181 - 2125 BC, belongs to the "First Intermediate Period"; collapse of central authority'),
        html.Br(),
        html.Div(id="grid-output_7"),
        dbc.Modal(id="custom-component-img-modal_7", size="s"),
        html.Div(grid_note),
        html.Br(),
    ],
    style={
...
# 
@callback(
    Output("grid-output_7", "children"),
    Input("store", "data"),
)
def update(store):
    ''' Update for seventh dynasty, having an empty dataframe '''
    if store == {}:
        return "Have you selected seventh dynasty dropdown item? Empty dataframe columns shall be visible ..."
    
    df_dyn = pd.DataFrame({})
    logger.debug('-----  in seventh dyn: callback update(store): store: %s -----', store)

    return dag.AgGrid(
                id='seventh_dynasty_img_dag',
