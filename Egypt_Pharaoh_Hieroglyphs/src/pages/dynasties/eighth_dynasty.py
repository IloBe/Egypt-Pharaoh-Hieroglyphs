...
layout = html.Div(
    children = [
        ...
        html.Br(),
        html.Div(id="grid-output_8"),
        dbc.Modal(id="custom-component-img-modal_8", size="s"),
        html.Div(grid_note),
        html.Br(),
    ],
    style={
...
# 
@callback(
    Output("grid-output_8", "children"),
    Input("store", "data"),
)
def update(store):
    ''' Update for eighth dynasty, having an empty dataframe '''
    if store == {}:
        return "Have you selected eighth dynasty dropdown item? Empty dataframe columns shall be visible ..."
    
    df_dyn = pd.DataFrame({})
    logger.debug('-----  in eighth dyn: callback update(store): store: %s -----', store)

    return dag.AgGrid(
                id='eighth_dynasty_img_dag',
